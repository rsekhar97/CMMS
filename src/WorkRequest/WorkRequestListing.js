...
const WorkRequestListing =({navigation,route})=>{
    ...
    const _goBack = () => {
        ...
        if(route.params.Screenname === 'FilteringWorkRequest'){
          navigation.navigate('FilteringWorkRequest',{ Screenname: route.params.Screenname })
    ...
    React.useEffect(() => {
      ...
      const focusHander = navigation.addListener('focus', ()=>{
        ...
        console.log('WR_test123',route.params)
    ...
    const get_workrequestlist =(async ()=>{
      ...
      if(route.params.Screenname === 'FilteringWorkRequest'){
          ...
          userStr = {site_cd:Site_cd,
            wkr_mst_wr_status:'Waiting',
            
            wkr_mst_wr_no:route.params.WRF_WorkrequestNo,
            wkr_mst_wr_descs:route.params.WRF_WorkrequestDesc,

            wkr_mst_assetno:route.params.WRF_AssetNo,
            ast_mst_asset_shortdesc:route.params.WRF_AssetDesc,
            wkr_mst_originator:route.params.WRF_Originator,
            wkr_mst_work_area:route.params.WRF_WorkArea,
            wko_mst_asset_location:route.params.WRF_AssetLocation,
            wko_mst_asset_level:route.params.WRF_AssetLevel,

            wkr_mst_chg_costcenter:route.params.WRF_CostCenter,
            wkr_mst_org_date:route.params.WRF_Fromdate,
            wkr_mst_due_date:route.params.WRF_Todate,
            createby:"",
      ...
      console.log("USE DATA Work Request Listing: "+JSON.stringify(userStr)) 

      try{

        const response = await axios.post(`${BaseUrl}/get_workrequest_list.php?`,JSON.stringify(userStr));
    ...
    const searchFilterFunction = (text) => {
      ...
      if (text) {
        ...
        const newData = WorkOrderList.filter(function (item) {
            ...
            const itemData = `${item.wkr_mst_assetno.toUpperCase()}
            ,${item.wkr_mst_wr_no.toUpperCase()}
            ,${item.wkr_mst_work_group.toUpperCase()}
            ,${item.wkr_mst_work_area.toUpperCase()}
            ,${item.wkr_mst_assetlocn.toUpperCase()}
            ,${item.wkr_mst_location.toUpperCase()}
            ,${item.wkr_mst_originator.toUpperCase()})`
    ...
    const ItemView = ({ item }) => {
     ...
     // console.log('DUE',item.wkr_mst_wr_no +':'+item.wkr_mst_due_date.date)
        ...
        <TouchableOpacity onPress={() => getItem(item)}>
          ...
          <View style={{ flexDirection: 'row', marginTop:5 }}>
              ...
              {/* <View style={{flex:1,justifyContent:'space-between',flexDirection:'row'}}>
                ...
                <Text style={{fontSize: 15,color:wrk_color,marginRight:10,fontWeight:'bold'}} >{wrk_status}</Text>
              </View>
    ...
    const getItem = (item) => {
      ...
      if(route.params.Screenname === 'FilteringWorkRequest'){
        ...
        navigation.navigate('CreateWorkRequest',{
    ...
    const Button_select_list =async (selected_value)=>{
      ...
      if(route.params.Screenname === 'FilteringWorkRequest'){
        ...
        userStr = {site_cd:Site_cd,
          wkr_mst_wr_status:selected_value,
    ...
    const goBack_filterpage = () =>{
      
      navigation.navigate("FilteringWorkRequest",{Screenname:route.params.Screenname})
    }

    const goBack_Create_WR = () =>{
      console.log('ok')
          ...
          <View style={{flexDirection:'row',flex:1,justifyContent:'space-between'}}>
            <Pressable onPress={_goBack}>
              <View style={{flexDirection:'row',alignItems:'center',}}>
                ...
                />  
                <Text style={{fontSize:20, justifyContent:'center',textAlign:'center',color:'#fff',fontWeight: 'bold',marginLeft:15}}> Work Request</Text> 
              </View >
            </Pressable>
            <View style={{flexDirection:'row',alignItems:'center'}}>
              <Pressable onPress={()=>goBack_filterpage()}>
              ...
              <Pressable onPress={()=>goBack_Create_WR()}>
                <View style={{justifyContent:'center',alignItems:'center',marginLeft:5,marginRight:5}}>
                  ...
                  <Image source={require('../../images/floating.png')}
                    style={{ width: 30, height: 30,marginLeft:10}}/>
                </View>
              </Pressable>
            </View>  
...
export default WorkRequestListing;
